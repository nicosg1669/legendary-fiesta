...
# Version 1.4 — Añadido listado detallado de DNSBLs encontradas en el reporte
# ==============================================================================
...
# Comprueba que existan las dependencias mínimas
check_deps() {
...
# ─── BLOQUE 1: GEO + WHOIS clásico con fallback ────── Nicolas Saavedra
...
show_whois() {
  ...
  local whois_out
  whois_out=$(whois "$1" 2>/dev/null \
               | grep -Ei 'CIDR|NetName|OrgName|Country' \
               | uniq)
  if [[ -n "$whois_out" ]]; then
    echo -e "   ${whois_out//$'\n'/"\n   "}"
  else
    echo -e "   ⚠️ WHOIS clásico no disponible o puerto 43 bloqueado. Usando RDAP…"
    local rdap=$(curl -s "https://rdap.arin.net/registry/ip/$1")
...
count=0
listed=()   # aquí guardaremos las DNSBL donde esté
